...
/* ══════ BASE64 NORMALIZER ══════ */
/* Accepts raw base64 OR full data URL ("data:image/jpeg;base64,...").
   Returns {data, media_type} or {error}. */
function normalizeImagePayload(b64OrDataUrl,fallbackMime){
  if(!b64OrDataUrl||typeof b64OrDataUrl!=="string"){return{error:"No image data provided"}}
  var data=b64OrDataUrl;var mime=fallbackMime||"image/jpeg";
  if(data.startsWith("data:")){
    var commaIdx=data.indexOf(",");
    if(commaIdx<0)return{error:"Malformed data URL — no comma found"};
    var prefix=data.slice(0,commaIdx);var mm=prefix.match(/^data:([^;]+)/);
    if(mm)mime=mm[1];
    data=data.slice(commaIdx+1);
  }
  data=data.replace(/\s/g,"");
  if(!data||data.length<100){return{error:"Image data too short or empty ("+data.length+" chars). Try re-uploading."}}
  var SUPPORTED=["image/jpeg","image/png","image/gif","image/webp"];
  if(SUPPORTED.indexOf(mime)<0)mime="image/jpeg";
  return{data:data,media_type:mime};
}

async function aiID(b64,mime,apiKey){
  _lastAiError="";
  var img=normalizeImagePayload(b64,mime);
  if(img.error){_lastAiError=img.error;return null}
  try{
    ...
    if(Array.isArray(p)){if(p.length&&p[0].garmentType&&p[0].color){
      ...
      var VALID_TYPES=["Shirt","Polo","T-Shirt","Henley","Tank Top","Sweater/Knit","Cardigan","Hoodie","Sweatshirt","Jacket/Blazer","Vest/Gilet","Overshirt","Coat","Pants/Trousers","Chinos","Jeans","Shorts","Shoes","Hat","Scarf","Belt","Bag","Sunglasses","Accessory"];
      var REJECT=["sock","socks","underwear","undershirt","boxers","briefs","bra","panties","undergarment"];
      p=p.filter(function(item){return !REJECT.some(function(r){return(item.garmentType||"").toLowerCase().includes(r)||(item.name||"").toLowerCase().includes(r)})});
      p.forEach(function(item){if(!VALID_TYPES.includes(item.garmentType)){item.garmentType="Shirt";item.needsEdit=true}if(!AC.includes(item.color)){item.needsEdit=true}});
      if(!p.length){_lastAiError="AI returned only excluded items (socks/underwear)";return null}
      return p}
    _lastAiError="AI returned empty array";return null}
...
export {
  getLastAiError, normalizeImagePayload, aiID, aiVision, aiOccasionPlan as aiOccasion,
