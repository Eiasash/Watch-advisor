...
function ph(s){if(!s)return null;if(s.startsWith("idb:")){var v=_pc[s];if(v)return v;/* Lazy resolve: trigger async fetch so next render picks it up */if(!_pc["_q"+s]){_pc["_q"+s]=1;idbOpen().then(function(db){var t=db.transaction("imgs","readonly");var r=t.objectStore("imgs").get(s.slice(4));r.onsuccess=function(){if(r.result){_pc[s]=URL.createObjectURL(r.result);/* Trigger React re-render via global event */window.dispatchEvent(new Event("wa-photo-ready"))}delete _pc["_q"+s]};r.onerror=function(){delete _pc["_q"+s]}}).catch(function(){delete _pc["_q"+s]})}return null}return s}
