    ...
    for(var i=0;i<arr.length;i++){
      ...
      try{
        ...
        if(aiResult&&Array.isArray(aiResult)&&aiResult.length>0){
          ...
          if(_dh){if(typeof console!=="undefined"){var _dists=wd.filter(function(e){return e.dHash}).map(function(e){return{name:e.name||e.color,dist:hammingDist(e.dHash,_dh)}}).sort(function(a,b){return a.dist-b.dist}).slice(0,5);console.log("[dHash] Nearest:",JSON.stringify(_dists))}var _dupeMatch=wd.find(function(existing){return existing.dHash&&hammingDist(existing.dHash,_dh)<=10});if(_dupeMatch){showToast("Possible duplicate of "+(_dupeMatch.name||_dupeMatch.color),"var(--gold)",4000)}}
          if(aiResult.length>1)showToast("Found "+aiResult.length+" items in photo","var(--gold)",3000);
        }else{
        ...
        console.warn("[WA] File error:",e);setAiErr("Couldn't read photo — try again.");
      }
    ...
    var GE=function(){
      ...
      var retry=async function(){
        ...
        try{var _raw=item.photoUrl.startsWith("idb:")?await photoAsDataUrl(item.photoUrl):item.photoUrl;if(!_raw){setRetrying(false);return}var compressed=await compressImage(_raw,800,0.7);var b64=compressed.split(",")[1];
          ...
          else showToast(getLastAiError()||"Couldn't identify — classify manually.","var(--sub)",3500)}catch(e){console.warn("[WA] Retry:",e);showToast("Couldn't classify — try manually.","var(--sub)",3000)}
        setRetrying(false);
      };
    ...
    React.createElement("div",{ref:mainRef,style:{maxWidth:860,margin:"0 auto",padding:"0 16px"}},
      ...
      view==="wardrobe"&&React.createElement("div",{key:"tab-wardrobe",className:"tab-content",style:{paddingTop:16,paddingBottom:40}},
        ...
        React.createElement("div",{style:{display:"flex",gap:8,marginBottom:14}},
          React.createElement("button",{onClick:function(){if(fRef.current)fRef.current.click()},style:{flex:1,background:"linear-gradient(135deg,var(--gold),#a8882a)",border:"none",borderRadius:10,padding:"14px 16px",color:"var(--bg)",fontFamily:"var(--f)",fontSize:13,fontWeight:600,cursor:"pointer",minHeight:48,display:"flex",alignItems:"center",justifyContent:"center",gap:6}},"+  Add Photos"),
          React.createElement("button",{onClick:function(){if(cRef.current)cRef.current.click()},style:{background:"var(--card)",border:"1px solid var(--border)",borderRadius:10,padding:"14px 16px",color:"var(--sub)",fontFamily:"var(--f)",fontSize:14,cursor:"pointer",minHeight:48}},"📷"),
          React.createElement("button",{onClick:function(){setAddG(true)},style:{background:"var(--card)",border:"1px solid var(--border)",borderRadius:10,padding:"14px 16px",color:"var(--sub)",fontFamily:"var(--f)",fontSize:14,cursor:"pointer",minHeight:48}},"✏️")),

        proc.length>0&&React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(115px,1fr))",gap:8,marginBottom:12}},proc.map(function(p){return React.createElement("div",{key:p,className:"gcard",style:{background:"var(--card)",border:"1px solid var(--border)",overflow:"hidden"}},React.createElement("div",{className:"skeleton",style:{width:"100%",height:100}}),React.createElement("div",{style:{padding:"10px 10px 8px"}},React.createElement("div",{className:"skeleton",style:{height:11,width:"70%",marginBottom:6,borderRadius:4}}),React.createElement("div",{className:"skeleton",style:{height:9,width:"50%",borderRadius:4}})))})),

        wd.length>0&&React.createElement(React.Fragment,null,
          (function(){if(needsFix<=0&&!aiErr)return null;var total=wd.length;var classified=total-needsFix;var pct=total>0?Math.round(classified/total*100):0;var broken=wd.filter(function(i){return(i.needsEdit||!i.color)&&i.photoUrl});return React.createElement("div",{style:{background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,padding:"14px 16px",marginBottom:12}},
            needsFix>0&&React.createElement(React.Fragment,null,
              React.createElement("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}},
                React.createElement("div",null,
                  React.createElement("div",{style:{fontSize:13,fontFamily:"var(--f)",fontWeight:600,color:"var(--text)"}},pct+"% classified"),
                  React.createElement("div",{style:{fontSize:10,fontFamily:"var(--f)",color:"var(--sub)",marginTop:2}},needsFix+" item"+(needsFix!==1?"s":"")+" to review")),
                React.createElement("div",{style:{display:"flex",gap:6}},
                  broken.length>0&&React.createElement("button",{onClick:async function(){
                    for(var k=0;k<broken.length;k++){var it=broken[k];
                  ...
                  },style:{background:"none",border:"1px solid var(--border)",borderRadius:8,padding:"8px 12px",color:"var(--sub)",fontFamily:"var(--f)",fontSize:10,cursor:"pointer",whiteSpace:"nowrap",minHeight:32}},"Auto-Retry"),
                  React.createElement("button",{onClick:function(){var first=wd.find(function(i){return i.needsEdit||!i.color});if(first)setEditG(first)},style:{background:"linear-gradient(135deg,var(--gold),#a8882a)",border:"none",borderRadius:8,padding:"8px 14px",color:"var(--bg)",fontFamily:"var(--f)",fontSize:11,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",minHeight:32}},"Review Next"))),
              React.createElement("div",{style:{height:4,borderRadius:2,background:"var(--bg)",overflow:"hidden"}},
                React.createElement("div",{style:{height:"100%",width:pct+"%",background:"var(--gold)",borderRadius:2,transition:"width .5s cubic-bezier(.22,.68,0,1)"}}))),
            aiErr&&React.createElement("div",{style:{marginTop:needsFix>0?10:0,display:"flex",alignItems:"center",gap:8}},
              React.createElement("span",{style:{fontSize:10,fontFamily:"var(--f)",color:"var(--sub)",flex:1}},aiErr),
              React.createElement("button",{onClick:function(){setAiErr("")},style:{background:"none",border:"none",color:"var(--dim)",cursor:"pointer",fontSize:12,padding:0}},"✕")))})(),
          ...
          React.createElement("div",{style:{display:"flex",gap:5,flexWrap:"wrap",marginBottom:10}},
            ...
            (function(){var archCount=wd.filter(function(i){return i.archived}).length;return archCount>0?React.createElement(Pill,{on:showArchived,onClick:function(){setShowArchived(!showArchived)}},(showArchived?"Hide":"Show")+" "+archCount+" archived"):null})()),
              ...
              var _isSel=selMode&&selIdsRef.current.has(item.id);return React.createElement("div",{key:item.id,className:"gcard",onClick:function(){if(selMode){toggleSel(item.id)}else{setEditG(item)}},onPointerDown:function(){if(!selMode){longPressTimer.current=setTimeout(function(){selIdsRef.current=new Set([item.id]);setSelMode(true);setSelTick(1)},500)}},onPointerUp:function(){clearTimeout(longPressTimer.current)},onPointerLeave:function(){clearTimeout(longPressTimer.current)},style:{background:_isSel?"rgba(201,168,76,0.12)":"var(--card)",border:"1px solid "+(_isSel?"var(--gold)":"var(--border)")}},
                ...
                React.createElement("div",{style:{padding:"8px 10px"}},
                  React.createElement("div",{style:{display:"flex",alignItems:"center",gap:4}},
                    ...
                    React.createElement("span",{style:{fontSize:11,fontFamily:"var(--f)",fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1}},item.name||item.color||"Unclassified")),
                ...
                (item.needsEdit||!item.color)&&React.createElement("div",{style:{position:"absolute",top:4,right:4,background:"rgba(0,0,0,0.55)",backdropFilter:"blur(4px)",WebkitBackdropFilter:"blur(4px)",borderRadius:6,padding:"2px 7px"}},React.createElement("span",{style:{fontSize:8,fontFamily:"var(--f)",color:"#ccc",fontWeight:500,letterSpacing:"0.02em"}},"Review")),
                selMode&&React.createElement("div",{style:{position:"absolute",top:4,left:4,width:22,height:22,borderRadius:"50%",background:_isSel?"var(--gold)":"rgba(0,0,0,0.5)",border:"2px solid "+(_isSel?"var(--gold)":"rgba(255,255,255,0.4)"),display:"flex",alignItems:"center",justifyContent:"center",zIndex:3}},_isSel&&React.createElement("span",{style:{color:"var(--bg)",fontSize:13,fontWeight:700}},"\u2713")),
                item.material&&React.createElement("div",{style:{position:"absolute",bottom:32,right:4,background:"rgba(0,0,0,0.55)",borderRadius:4,padding:"2px 5px"}},React.createElement("span",{style:{fontSize:8,fontFamily:"var(--f)",color:"#aaa"}},item.material)),
                item.lastWorn&&React.createElement("div",{style:{position:"absolute",bottom:32,left:4,background:"rgba(0,0,0,0.55)",borderRadius:4,padding:"2px 5px"}},React.createElement("span",{style:{fontSize:8,fontFamily:"var(--f)",color:(function(){var d=Math.floor((Date.now()-new Date(item.lastWorn).getTime())/864e5);return d<=2?"#e8a87c":d<=7?"#aaa":"#7a7"})()}},(function(){var d=Math.floor((Date.now()-new Date(item.lastWorn).getTime())/864e5);return d===0?"today":d===1?"1d ago":d+"d ago"})())),
          ...
          /* Batch action bar */
          selMode&&selIdsRef.current.size>0&&React.createElement("div",{style:{display:"flex",gap:6,marginTop:10,padding:"12px 14px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,alignItems:"center",flexWrap:"wrap"}},
            React.createElement("span",{style:{fontSize:12,fontFamily:"var(--f)",color:"var(--text)",fontWeight:600,flex:1,minWidth:80}},selIdsRef.current.size+" selected"),
            React.createElement("button",{onClick:function(){selIdsRef.current=new Set(filteredWd.map(function(i){return i.id}));setSelTick(function(t){return t+1})},style:{background:"none",border:"1px solid var(--border)",borderRadius:8,padding:"8px 12px",color:"var(--sub)",fontFamily:"var(--f)",fontSize:10,cursor:"pointer",minHeight:32}},"Select All"),
            React.createElement("button",{onClick:batchArchive,style:{background:"none",border:"1px solid var(--border)",borderRadius:8,padding:"8px 12px",color:"var(--sub)",fontFamily:"var(--f)",fontSize:10,cursor:"pointer",minHeight:32}},"Archive"),
            React.createElement("button",{onClick:function(){setBatchTypeMenu(!batchTypeMenu)},style:{background:"none",border:"1px solid var(--border)",borderRadius:8,padding:"8px 12px",color:"var(--sub)",fontFamily:"var(--f)",fontSize:10,cursor:"pointer",minHeight:32}},"Reclassify"),
            React.createElement("button",{onClick:batchDelete,style:{background:"none",border:"1px solid var(--border)",borderRadius:8,padding:"8px 12px",color:"var(--warn)",fontFamily:"var(--f)",fontSize:10,cursor:"pointer",minHeight:32}},"Delete "+selIdsRef.current.size)),
          ...
          React.createElement("div",{style:{background:"var(--card)",border:"1px solid var(--border)",borderRadius:10,padding:"10px 14px",marginTop:12}},
            ...
            wdStats.unused>0&&React.createElement("p",{style:{fontSize:9,fontFamily:"var(--f)",color:"var(--dim)",margin:"4px 0 0"}},wdStats.unused+" item"+(wdStats.unused!==1?"s":"")+" not yet in a saved outfit"),
            /* Freshness bar */
            (function(){if(!wdStats.hasWorn)return null;var fresh=wdStats.fresh,mid=wdStats.mid,stale=wdStats.stale,total=fresh+mid+stale;if(!total)return null;return React.createElement("div",{style:{marginTop:6}},React.createElement("div",{style:{display:"flex",gap:8,fontSize:8,fontFamily:"var(--f)",color:"var(--sub)",marginBottom:3}},React.createElement("span",null,"Rotation:"),fresh>0&&React.createElement("span",{style:{color:"var(--good)"}},fresh+" fresh"),mid>0&&React.createElement("span",{style:{color:"var(--gold)"}},mid+" ok"),stale>0&&React.createElement("span",{style:{color:"var(--dim)"}},stale+" stale")),React.createElement("div",{style:{height:3,borderRadius:2,background:"var(--bg)",overflow:"hidden",display:"flex"}},fresh>0&&React.createElement("div",{style:{width:fresh/total*100+"%",background:"var(--good)"}}),mid>0&&React.createElement("div",{style:{width:mid/total*100+"%",background:"var(--gold)"}}),stale>0&&React.createElement("div",{style:{width:stale/total*100+"%",background:"var(--dim)"}})))})()))),
