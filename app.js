    ...
    showWDupes&&React.createElement(Modal,{onClose:function(){setShowWDupes(false)}},
              ...
              group.map(function(w){
                return React.createElement("div",{key:w.id,style:{background:"var(--card)",borderRadius:10,padding:"12px",border:"1px solid var(--border)"}},
                  React.createElement("div",{style:{display:"flex",alignItems:"center",gap:8,marginBottom:8}},
                    React.createElement("div",{style:{width:36,height:36,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",background:w.c+"20",fontSize:18,border:"2px solid "+w.c+"60"}},w.photoUrl?React.createElement("img",{src:ph(w.photoUrl),alt:"",style:{width:"100%",height:"100%",objectFit:"cover",borderRadius:"50%"}}):w.i||"⌚"),
                    React.createElement("div",{style:{flex:1}},
      ...
      React.createElement("div",{style:{marginTop:16,paddingTop:12,borderTop:"1px solid var(--border)"}},
        ...
        React.createElement("div",{style:{display:"flex",gap:8,marginBottom:8}},
          ...
          React.createElement("button",{onClick:async function(){
            ...
            try{
              var data=JSON.stringify({version:"v25.2",user:recUser.trim(),ts:Date.now(),watches:W,wardrobe:wd,outfits:saved,wearLog:wearLog,weekCtx:weekCtx,userCx:userCx,rotLock:rotLock,selfieHistory:selfieHistory,theme:theme});
      ...
      React.createElement("div",{style:{marginTop:0,paddingTop:12,borderTop:"1px solid var(--border)"}},
        ...
        React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap",marginTop:6}},
          React.createElement("button",{className:"btn btn-ghost",style:{flex:1,fontSize:11,padding:"10px 14px",minHeight:40},onClick:async function(){try{var ew=JSON.parse(JSON.stringify(W)),ewd=JSON.parse(JSON.stringify(wd));for(var it of ewd){if(it.photoUrl&&it.photoUrl.startsWith("idb:"))it.photoUrl=await photoAsDataUrl(it.photoUrl)||null}for(var w of ew){if(w.photoUrl&&w.photoUrl.startsWith("idb:"))w.photoUrl=await photoAsDataUrl(w.photoUrl)||null;if(w.straps)for(var s of w.straps){if(s.photoUrl&&s.photoUrl.startsWith("idb:"))s.photoUrl=await photoAsDataUrl(s.photoUrl)||null}}var data=JSON.stringify({version:"v25.2",watches:ew,wardrobe:ewd,outfits:saved,wearLog:wearLog,weekCtx:weekCtx,userCx:userCx,rotLock:rotLock,selfieHistory:selfieHistory,theme:theme});var blob=new Blob([data],{type:"application/json"});var a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="watch-advisor-backup.json";a.click()}catch(e){console.error("[Export]",e)}}},"‍📤 Export"),
          ...
          React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(105px,1fr))",gap:8}},
            filteredWd.slice(0,gridLimit).map(function(item){
              var _isSel=selMode&&selIdsRef.current.has(item.id);return React.createElement("div",{key:item.id,className:"gcard",onClick:function(){if(selMode){toggleSel(item.id)}else{setEditG(item)}},onPointerDown:function(){if(!selMode){longPressTimer.current=setTimeout(function(){selIdsRef.current=new Set([item.id]);setSelMode(true);setSelTick(1)},500)}},onPointerUp:function(){clearTimeout(longPressTimer.current)},onPointerLeave:function(){clearTimeout(longPressTimer.current)},style:{background:_isSel?"rgba(201,168,76,0.12)":item.needsEdit||!item.color?"var(--card2)":"var(--card)",border:"1px solid "+(_isSel?"var(--gold)":item.needsEdit||!item.color?"rgba(201,168,76,0.3)":"var(--border)")}},
                ...
                (item.needsEdit||!item.color)&&React.createElement("div",{style:{position:"absolute",top:4,right:4,background:"rgba(201,168,76,0.9)",borderRadius:4,padding:"2px 6px"}},React.createElement("span",{style:{fontSize:8,fontFamily:"var(--f)",color:"var(--bg)",fontWeight:700}},"FIX")),selMode&&React.createElement("div",{style:{position:"absolute",top:4,left:4,width:20,height:20,borderRadius:"50%",background:_isSel?"var(--gold)":"rgba(0,0,0,0.5)",border:"2px solid "+(_isSel?"var(--gold)":"rgba(255,255,255,0.4)"),display:"flex",alignItems:"center",justifyContent:"center",zIndex:3}},_isSel&&React.createElement("span",{style:{color:"var(--bg)",fontSize:12,fontWeight:700}},"\u2713")),
                item.positionHint&&React.createElement("div",{style:{position:"absolute",top:4,left:4,background:"rgba(0,0,0,0.7)",borderRadius:4,padding:"1px 5px"}},React.createElement("span",{style:{fontSize:7,fontFamily:"var(--f)",color:"var(--sub)"}},"📸 "+item.positionHint)),
                item.photoUrl&&React.createElement("button",{onClick:function(e){e.stopPropagation();var inp=reScanRef.current;if(!inp)return;inp.onchange=async function(ev){var fl=ev.target.files[0];if(!fl)return;try{var raw=await new Promise(function(r){var x=new FileReader();x.onload=function(){r(x.result)};x.readAsDataURL(fl)});var compressed=await compressImage(raw,1200,0.82);updG(item.id,{photoUrl:compressed});inp.value=""}catch(err){console.error(err)}};inp.click()},style:{position:"absolute",bottom:28,right:24,background:"rgba(0,0,0,0.65)",border:"none",borderRadius:4,padding:"2px 5px",cursor:"pointer",color:"#fff",fontSize:8,zIndex:2,lineHeight:1},title:"Re-photograph"},"📸"),
        ...
        (function(){var now=Date.now();var stale=wd.filter(function(i){if(!i.lastWorn||i.archived)return false;var d=Math.floor((now-new Date(i.lastWorn).getTime())/864e5);return d>=14});stale.sort(function(a,b){return new Date(a.lastWorn).getTime()-new Date(b.lastWorn).getTime()});return stale.length>0?React.createElement("div",{style:{background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,padding:"16px",marginBottom:16}},
          ...
          React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(90px,1fr))",gap:6}},
            stale.slice(0,12).map(function(item){var d=Math.floor((now-new Date(item.lastWorn).getTime())/864e5);return React.createElement("div",{key:item.id,className:"gcard",onClick:function(){setEditG(item)},style:{background:"var(--bg)",border:"1px solid rgba(200,90,58,0.2)"}},
              item.photoUrl?React.createElement("img",{src:ph(item.photoUrl),alt:"",decoding:"async",style:{width:"100%",height:60,objectFit:"cover",display:"block",opacity:0.7}}):React.createElement("div",{style:{width:"100%",height:40,background:(CM[item.color]||{}).h||"#2a2a2a",opacity:0.7}}),
